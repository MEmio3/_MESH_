...
export function updateServerChannelSettings(id: string, bitrateKbps: number | null, userLimit: number): void {
  getDb().prepare('UPDATE server_channels SET bitrate_kbps = ?, user_limit = ? WHERE id = ?').run(bitrateKbps, userLimit, id)
}

export function updateServerChannelSendRoles(id: string, sendRoleIdsJson: string | null): void {
  getDb().prepare('UPDATE server_channels SET send_role_ids = ? WHERE id = ?').run(sendRoleIdsJson, id)
}
